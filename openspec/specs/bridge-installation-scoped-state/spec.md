# bridge-installation-scoped-state Specification

## Purpose
Define installation ownership and the historical version-1-to-version-2 migration. Current schema creation and message-state migration are governed by bridge-conversation-scoped-message-state; version 2 below denotes the historical migration stage, not the current activation target.
