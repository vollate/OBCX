...
The bridge retry worker SHALL resolve target bots through the process-owned
`BotRegistry` shared with the actor generation. It MUST support QQ group sends,
Telegram group sends, and Telegram topic sends, and MUST accept a retry as
successful only after extracting the target platform's message id from a valid
response.

#### Scenario: Telegram group retry succeeds
- **WHEN** a QQ-origin retry targets a Telegram group without a positive topic id and Telegram returns `result.message_id`
- **THEN** the worker records that message id as the target id and completes the retry

#### Scenario: Telegram topic retry succeeds
- **WHEN** a QQ-origin retry has a positive Telegram topic id and Telegram returns `result.message_id`
- **THEN** the worker sends through the topic API, records the returned message id, and completes the retry

#### Scenario: QQ group retry succeeds
- **WHEN** a Telegram-origin retry targets a QQ group and QQ returns `data.message_id`
- **THEN** the worker records that message id as the target id and completes the retry

#### Scenario: Target response is unusable
- **WHEN** the target bot is unavailable, throws, returns no response, or returns a response without the expected message id
...
The bridge actor SHALL derive message maximum attempts, base backoff, queue
check interval, and maximum backoff from its immutable generation
configuration. Attempt and interval values MUST be positive, base and check
intervals MUST NOT exceed the maximum interval, and invalid settings MUST fail
actor-aware configuration validation.

#### Scenario: Retry is rescheduled
- **WHEN** a resend attempt fails and has not reached its configured maximum
...
- **WHEN** a failed resend reaches its configured maximum attempt count
