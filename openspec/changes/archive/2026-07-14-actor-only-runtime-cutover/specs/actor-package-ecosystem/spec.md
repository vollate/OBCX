...
The OBCX cutover release SHALL pin and verify compatible bridge,
message-store, and actor-registry revisions. Core removal SHALL NOT be marked
complete until every required repository passes its actor-only build and smoke
tests against the installed SDK.
