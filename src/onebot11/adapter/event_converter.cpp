...
auto EventConverter::from_v11_json(std::string_view json_str)
  ...
  if (!j_opt) {
    OBCX_KEY_WARN(common::LogMessageKey::ONEBOT11_EVENT_PARSE_JSON_FAILED,
                  json_str);
    return std::nullopt;
  }
  ...
  try {
    ...
    if (post_type == "meta_event") {
      ...
      if (meta_event_type == "heartbeat") {
        ...
        event.from_json(j);
        OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_EVENT_HEARTBEAT,
                       event.interval);
        return event;
      } else {
    ...
    OBCX_KEY_ERROR(common::LogMessageKey::ONEBOT11_EVENT_CREATE_EXCEPTION,
                   e.what(), json_str);
    return std::nullopt;
  }

  OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_EVENT_UNKNOWN_POST_TYPE,
                 post_type);
  return std::nullopt;
}
