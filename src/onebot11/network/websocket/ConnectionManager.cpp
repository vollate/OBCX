...
auto WebSocketConnectionManager::send_action_and_wait_async(
  ...
  constexpr bool USE_EVENT_DRIVEN_TIMEOUT = true;
  ...
  if constexpr (USE_EVENT_DRIVEN_TIMEOUT) {
    ...
    completion_semaphore->expires_at(std::chrono::steady_clock::time_point::max());

    // 设置resolver - 纯事件驱动，完成时唤醒协程
    request->resolver = [state, echo_id, completion_semaphore](std::string response) {
      OBCX_DEBUG("Resolver调用，echo: {}, response: {}", echo_id, response);
      {
        std::lock_guard<std::mutex> lock(state->state_mutex);
        state->result = std::move(response);
      }
      completion_semaphore->cancel(); // 唤醒等待的协程
      OBCX_DEBUG("响应已设置并唤醒协程，echo: {}", echo_id);
    };

    request->rejecter = [state, completion_semaphore](std::exception_ptr ex) {
      {
        std::lock_guard<std::mutex> lock(state->state_mutex);
        state->error_ptr = std::move(ex);
      }
      completion_semaphore->cancel(); // 错误时也要唤醒协程
    };
    ...
    try {
      ...
      // 启动超时检查 - 超时时唤醒协程
      asio::co_spawn(ioc_,
        [request, state, echo_id, completion_semaphore]() -> asio::awaitable<void> {
          try {
            ...
            completion_semaphore->cancel(); // 超时时唤醒协程
          } catch (const boost::system::system_error& e) {
            if (e.code() != asio::error::operation_aborted) {
              OBCX_ERROR("超时任务异常，echo: {}, error: {}", echo_id, e.what());
            }
          }
        }, asio::detached);
      ...
      try {
        ...
        // 如果没有抛出异常，说明计时器正常到期（不应该发生）
        OBCX_WARN("Completion semaphore正常到期，这不应该发生，echo: {}", echo_id);
      } catch (const boost::system::system_error& e) {
        if (e.code() == asio::error::operation_aborted) {
          // 正常情况：计时器被取消，说明响应到达或超时
      ...
      // 检查结果
      {
        ...
        OBCX_DEBUG("事件驱动API请求成功完成，echo: {}, result length: {}", echo_id,
                   state->result.length());
