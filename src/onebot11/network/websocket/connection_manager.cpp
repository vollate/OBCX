...
void WebSocketConnectionManager::disconnect() {
  ...
  // tear down strands/io_context.
  {
    ...
    OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_PENDING_CLEARED);
  }
...
void WebSocketConnectionManager::connect_ws(std::string host, uint16_t port,
  ...
  if (is_running_) {
    OBCX_KEY_WARN(common::LogMessageKey::ONEBOT11_WS_ALREADY_RUNNING);
    return;
  }
  host_ = std::move(host);
  port_ = port;
...
void WebSocketConnectionManager::do_connect() {
  asio::post(send_strand_, [this]() -> void {
    ...
    OBCX_KEY_INFO(common::LogMessageKey::WEBSOCKET_ATTEMPTING_CONNECTION, host_,
                  port_);
...
void WebSocketConnectionManager::on_ws_message(const beast::error_code &ec,
  ...
  OBCX_KEY_TRACE(common::LogMessageKey::ONEBOT11_WS_MSG_RECEIVED, message);
  if (ec) {
    OBCX_KEY_ERROR(common::LogMessageKey::ONEBOT11_WS_DISCONNECTED_ERROR,
                   ec.message());
    {
      is_connected_.store(false, std::memory_order_release);
    }
  ...
  if (message.empty()) {
    OBCX_KEY_INFO(common::LogMessageKey::WEBSOCKET_CONNECTION_ESTABLISHED);
    {
  ...
  OBCX_KEY_TRACE(common::LogMessageKey::ONEBOT11_WS_RAW_MSG, message);
  ...
  try {
    ...
    if (j.contains("echo") && j.contains("retcode")) {
      ...
      std::scoped_lock lock(pending_requests_mutex_);
      OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_FIND_PENDING, echo,
      ...
      if (it != pending_requests_.end()) {
        ...
        if (request->completion_handler) {
          OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_CALL_COMPLETION,
                         echo);
          request->completion_handler(boost::system::error_code{}, message);
        } else {
          OBCX_KEY_ERROR(common::LogMessageKey::ONEBOT11_WS_COMPLETION_NULL,
                         echo);
        }
        OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_API_RESPONSE_HANDLED,
                       echo);
        return;
      }
      OBCX_KEY_WARN(common::LogMessageKey::ONEBOT11_WS_UNKNOWN_API_RESPONSE,
                    echo);
      ...
      OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_CURRENT_PENDING,
                     pending_echos.str());
#endif
    }
  } catch (const nlohmann::json::exception &e) {
    OBCX_KEY_WARN(common::LogMessageKey::ONEBOT11_WS_JSON_PARSE_FAILED,
                  e.what());
  }
  ...
  if (event_opt) {
    ...
    OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_INVALID_EVENT);
  }
}

void WebSocketConnectionManager::schedule_reconnect() {
  reconnect_timer_.expires_after(std::chrono::seconds(5));
  OBCX_KEY_INFO(common::LogMessageKey::ONEBOT11_WS_RECONNECT_SCHEDULED, 5000);
  reconnect_timer_.async_wait([this](const beast::error_code &ec) -> void {
    if (ec) {
      if (ec != asio::error::operation_aborted) {
        OBCX_KEY_ERROR(common::LogMessageKey::ONEBOT11_WS_RECONNECT_TIMER_ERROR,
                       ec.message());
      }
      return;
    }
...
auto WebSocketConnectionManager::send_action_and_wait_async(
  ...
  OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_USE_COROUTINE);
...

  {
    ...
    OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_ADD_PENDING_COROUTINE,
  ...
  try {
    ...
    OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_MSG_SENT_COROUTINE,
                   echo_id);
    ...
    if (request->need_wait.load(std::memory_order_acquire)) {
      try {
        ...
        OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_REQUEST_TIMEOUT,
                       echo_id);
...

    {
      ...
      OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_CLEAN_PENDING_COROUTINE,
                     echo_id, pending_requests_.size());
    }

    {
      ...
      if (response_result) {
        OBCX_KEY_DEBUG(common::LogMessageKey::ONEBOT11_WS_API_SUCCESS_COROUTINE,
