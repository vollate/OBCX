...
TGBot::TGBot(adapter::telegram::ProtocolAdapter adapter,
  ...
  OBCX_KEY_INFO(common::LogMessageKey::TELEGRAMBOT_INSTANCE_CREATED);
}

TGBot::~TGBot() {
  OBCX_KEY_INFO(common::LogMessageKey::TELEGRAMBOT_INSTANCE_DESTROYED);
}
...
void TGBot::run() {
  ...
  OBCX_KEY_INFO(common::LogMessageKey::TELEGRAMBOT_STARTING_EVENT_LOOP);
  io_context_->run();
  OBCX_KEY_INFO(common::LogMessageKey::TELEGRAMBOT_EVENT_LOOP_ENDED);
}

void TGBot::stop() {
  OBCX_KEY_INFO(common::LogMessageKey::TELEGRAMBOT_REQUESTING_STOP);
...
auto TGBot::poll_updates() -> asio::awaitable<void> {
  ...
  while (is_connected()) {
    ...
    try {
      ...
      OBCX_KEY_DEBUG(common::LogMessageKey::TELEGRAMBOT_RECEIVED_UPDATES,
                     updates.length());
      ...
      OBCX_KEY_ERROR(common::LogMessageKey::TELEGRAMBOT_POLLING_ERROR,
                     e.what());
    }
...
auto TGBot::delete_message(std::string_view message_id)
  ...
  if (pos == std::string::npos) {
    throw std::invalid_argument(common::LogMessages::get_message(
...
auto TGBot::get_message(std::string_view message_id)
    -> asio::awaitable<std::string> {
  OBCX_KEY_WARN(common::LogMessageKey::TELEGRAMBOT_FUNCTION_NOT_IMPLEMENTED,
                "get_message");
  co_return "{}";
}
  ...
  try {
    if (message_data.contains("photo") && message_data["photo"].is_array() &&
      ...
      if (!largest_photo.is_null() && largest_photo.contains("file_id")) {
        ...
        OBCX_KEY_DEBUG(common::LogMessageKey::TELEGRAMBOT_PHOTO_CONTENT,
                       largest_photo.dump());
    ...
    for (const auto &media_type : media_types) {
      if (message_data.contains(media_type) &&
        ...
        if (media_obj.contains("file_id")) {
          ...
          OBCX_KEY_DEBUG(common::LogMessageKey::TELEGRAMBOT_MEDIA_CONTENT,
    ...
    OBCX_KEY_ERROR(common::LogMessageKey::TELEGRAMBOT_EXTRACT_MEDIA_ERROR,
                   e.what());
  }
...
auto TGBot::get_media_download_url(const MediaFileInfo &media_info)
  ...
  try {
    ...
    OBCX_KEY_ERROR(common::LogMessageKey::TELEGRAMBOT_GET_DOWNLOAD_URL_FAILED,
